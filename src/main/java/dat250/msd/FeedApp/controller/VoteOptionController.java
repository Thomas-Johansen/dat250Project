...
public class VoteOptionController {
    ...
    @GetMapping("/vote-option/{topicId}")
    public ResponseEntity<List<VoteOption>> getVoteOptions(@PathVariable Long topicId) {
        Topic topic = feedAppService.getTopicRepository().getTopicById(topicId);
        if (topic == null) {
            return feedAppService.createMessageResponse("VoteOption Get Failed: No topic with id: "+ topicId, HttpStatus.NOT_FOUND);
        }
    ...
    public ResponseEntity<VoteOption> deleteVoteOption(@RequestHeader(name = "Authorization") String sessionId, @PathVariable Long id) {
        ...
        topic.getVoteOptions().remove(option);
